...
int main(){
  ...
  char *tok;
  bool diabolico, esoterico, *array_serie;
  /*char fila[MAX_STR];*/
  string matrix;
  while(cin >> c && c!=0){
    cin.ignore();
    esoterico=true;
    array_serie=(bool*)calloc(c*c, sizeof(bool));
    getline(cin, matrix);
    tok=strtok(&matrix[0], " ");
    for(diabolico=true, i=aux_l=aux_d=CM=CM2=0; i!=c; i++){ //primero guardo el cuadrado en una matriz
      for(j=aux_l=0; j!=c && diabolico==true; j++, tok=strtok(NULL, " ")){ /*si diabolico esta a false no tiene sentido continuar*/
        if((cuadrado[i][j]=atoll(tok))>c*c || array_serie[cuadrado[i][j]]==true)
          esoterico=false;
        array_serie[cuadrado[i][j]]=true;
        ...
        if(i==j){
          aux_d+=cuadrado[i][j]; /*tambien guarda la CMa parcial de la diagonal*/
        }
      }
      if(diabolico==false){
        break;
